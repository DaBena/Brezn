...
# Ensure CocoaPods installed workspace exists
if [ ! -f "Brezn.xcworkspace/contents.xcworkspacedata" ]; then
    print_status "WARNING" "CocoaPods workspace not found. Run 'pod install' on macOS before building."
    if command -v pod &> /dev/null; then
        print_status "INFO" "Attempting 'pod install'..."
        pod install || print_status "WARNING" "pod install failed. Please run manually on macOS."
    fi
fi

# Clean previous builds
if command -v xcodebuild &> /dev/null; then
  xcodebuild clean -workspace Brezn.xcworkspace -scheme Brezn || true
fi
...
if command -v xcodebuild &> /dev/null; then
  ...
  print_status "ERROR" "xcodebuild not available. Run this step on macOS with Xcode."
fi
